<template>
  <div id="s_shape" title="S-Shape"
    ...
    <div class="a1" draggable="true" @dragstart="dragHalt"/>
    ...
    <div class="c2" draggable="true" @dragstart="dragHalt"/>
  </div>
</template>
...
#s_shape {
  ...
  position: absolute;
  display: grid;
  grid-template-columns: calc(var(--ss) + 1px) var(--ss) calc(var(--ss) + 1px);
  grid-template-rows: var(--ss) calc(var(--ss) + 1px);
  grid-template-areas:
    "a1 b1 c1"
    "a2 b2 c2";
  grid-gap: 0px;
...
.a2 {
  ...
  position: relative;
  z-index: 1;
  border-top: 1px solid White; /* simulates grid gap */
}
.a2.dragging {
  border-bottom: 1px solid White;
  border-left: 1px solid White;
