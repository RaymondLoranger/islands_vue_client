...
const title = (state, scoreId) =>
  ...
  `<span style="color:#ff0088;">${state.scores[scoreId].name}</span>'s Board` :
  `<span style="color:#1a1aff;">${state.scores[scoreId].name}</span>'s Board`
...
const getters = {
  ...
  haveBothPlayersSetIslands: state =>
    !['', 'initialized', 'players_set'].includes(state.gameState),
